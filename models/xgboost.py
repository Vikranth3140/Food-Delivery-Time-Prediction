...
X_train, X_test, y_train, y_test = train_test_split(features, target, test_size=0.2, random_state=42)#splitting

# Train the XGBoost model
model = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=500, max_depth=7, learning_rate=0.1)
...
accuracy_like = (abs(y_pred - y_test) <= tolerance).mean() * 100

print(f"Mean Squared Error: {mse:.4f}")
print(f"R2 Score: {r2:.4f}")
print(f"Accuracy-like Measure (within {tolerance} min): {accuracy_like:.2f}%")
with open("xgboost_metrics.txt", "w") as f:
    f.write(f"Mean Squared Error: {mse:.4f}\n")
    f.write(f"R2 Score: {r2:.4f}\n")
    f.write(f"Accuracy-like Measure (within {tolerance} min): {accuracy_like:.2f}%\n")